...
import { adminAPI, standingsAPI, matchAPI, fixtureAPI } from '../services/api'
...
const AdminDashboard = () => {
  ...
  const isLoading = statsLoading || standingsLoading || seasonLoading || recentLoading || upcomingLoading
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <StatCard title="Upcoming Fixtures" value={stats.upcomingFixtures || 0} icon={Activity} color="blue" />
                  ...
                  <div key={m.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                    <div className="min-w-0">
                      <p className="font-semibold text-sm text-gray-900">
                        ...
                        <span className="font-bold text-primary-600">{m.home_score} – {m.away_score}</span>{' '}
      ...
      <div className="card overflow-hidden">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs text-gray-400">Top {Math.min(standings.length, 10)} teams</span>
        </div>
                  ...
                  {standings.slice(0, 10).map((row, idx) => (
                    <tr key={row.id}
                      ...
                      <td className="py-2.5">
                        <div className="flex items-center gap-2">
                          <div className={`w-2 h-2 rounded-full flex-shrink-0
                            ${idx < 3 ? 'bg-green-400' : idx >= standings.length - 2 ? 'bg-red-400' : 'bg-gray-200'}`} />
                      ...
                      <td className={`py-2.5 text-center font-medium
                        ${row.goal_difference > 0 ? 'text-green-600' : row.goal_difference < 0 ? 'text-red-500' : 'text-gray-400'}`}>
